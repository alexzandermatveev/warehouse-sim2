...
public class Distribution {
    ...
    public static double distributeProductsRandomly(Warehouse warehouse, List<Product> products) {
...
//            System.out.printf("Товар %s распределён в ячейку %s%n", product.getId(), cell.getId());
     ...
     */
    public static double distributeWithELECTRE_TRI(Warehouse warehouse, List<Product> products) {
        List<Cell> cells = warehouse.getCells();
        if (products.size() > cells.size()) {
            throw new IllegalStateException("Недостаточно ячеек для всех товаров");
        }
...
//            System.out.printf("Товар %s распределён в ячейку %s (ELECTRE TRI)%n", product.getId(), bestCell.getId());
        ...
        return calculateObjectiveFunction(electreTriSolution, warehouse);
    }
     ...
     */
    public static double distributeWithTOPSIS(Warehouse warehouse, List<Product> products) {
        List<Cell> cells = warehouse.getCells();
        if (products.size() > cells.size()) {
            throw new IllegalStateException("Недостаточно ячеек для всех товаров");
        }
...
//            System.out.printf("Товар %s распределён в ячейку %s (TOPSIS)%n", product.getId(), cell.getId());
        ...
        // Рассчитываем целевую функцию
        return calculateObjectiveFunction(topsisSolution, warehouse);
    }
...
//        double[][] normalized = new double[rows][cols];
    ...
    public static double distributeWithELECTRE_TRI_and_TOPSIS(Warehouse warehouse, List<Product> products) {
        List<Cell> cells = warehouse.getCells();
        if (products.size() > cells.size()) {
            throw new IllegalStateException("Недостаточно ячеек для всех товаров");
        }
...
//                System.out.printf("Товар %s распределён в ячейку %s (ELECTRE TRI + TOPSIS)%n", product.getId(), bestCell.getId());
        ...
        return calculateObjectiveFunction(combinedSolution, warehouse);
    }
